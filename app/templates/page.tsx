    ...
    useEffect(() => {
        async function fetchTemplates() {
            try {
                ...
                const res = await fetch('https://smart-admin-api.onrender.com/admin/api/articles?filters[type][$eq]=Шаблон&populate=*', {
                    cache: 'no-store'
                });
                ...
                if (json.data) {
                    const formattedDocs = json.data.map((item: any) => {
                        ...
                        const fileUrl = fileData?.url ? `https://smart-admin-api.onrender.com/admin${fileData.url}` : "#";
